...
class Slot:
    ...
    the mp_length_impl slot fn is the raw ``mp_length_impl`` getter, not ``__len__``'s
...
def _tp_type(obj_type: type) -> PyTypeObject:
    ...
    # cross-group collapse, so a type with both sq_length_impl and mp_length_impl shows up in
...
class VariableTracker(metaclass=VariableTrackerMeta):
    ...
    def sq_length_impl(self, tx: InstructionTranslatorBase) -> VariableTracker:
        """Called when sq_length_impl is not implemented."""
        raise_observed_exception(
            TypeError,
            tx,
    ...
    def mp_length_impl(self, tx: InstructionTranslatorBase) -> VariableTracker:
        """Called when mp_length_impl is not implemented."""
        raise_observed_exception(
            TypeError,
            tx,
            args=[f"object of type '{self.python_type_name()}' has no len()"],
    ...
    def sq_contains_impl(
        ...
        """Called when sq_contains_impl is not implemented."""
        unimplemented(
            gb_type="missing sq_contains_impl",
